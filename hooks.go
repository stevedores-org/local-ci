...
# Run local-ci with fmt and clippy (fast checks)
if ! local-ci fmt clippy; then
  echo ""
  echo "❌ Pre-commit checks failed. Fix the issues above and try again."
  echo ""
  echo "💡 Tip: Run 'local-ci --fix fmt' to auto-fix formatting issues."
  exit 1
fi
...
func RemovePreCommitHook(root string) error {
	...
	if content == preCommitHookTemplate {
